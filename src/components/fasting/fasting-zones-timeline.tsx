    ...
    <div className="bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden">
      ...
      <div className="bg-gradient-to-r from-indigo-500 to-purple-600 p-6 text-white">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-xl font-bold mb-1">Metabolic Journey</h3>
            <p className="text-indigo-100 text-sm">Track your body's transformation</p>
          </div>
          <div className="text-right">
            <div className="text-3xl font-bold">{Math.floor(hoursElapsed)}h</div>
      ...
      {currentZone && (
        <div className={`${currentZone.bgColor} border-b-2 ${currentZone.borderColor} p-6`}>
          <div className="flex items-start gap-4">
            <div className={`${currentZone.color} bg-white rounded-full p-3 shadow-md`}>
              <currentZone.icon className="h-8 w-8" />
            </div>
            <div className="flex-1">
              <div className="flex items-center justify-between mb-2">
                <h4 className={`text-xl font-bold ${currentZone.color}`}>
                  {currentZone.name}
                </h4>
              ...
              <p className="text-gray-700 mb-3">{currentZone.description}</p>
                ...
                <ul className="space-y-1">
                  {currentZone.benefits.map((benefit, idx) => (
                    <li key={idx} className="text-sm text-gray-700 flex items-start gap-2">
                      <span className={`${currentZone.color} mt-1`}>•</span>
                      <span>{benefit}</span>
                    </li>
                  ))}
                </ul>
